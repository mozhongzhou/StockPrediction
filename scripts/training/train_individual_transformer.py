# scripts/training/train_individual_transformer.py
import os
import torch
...
class StockDataset(Dataset):
    def __init__(self, ticker, kline_type="weekly", seq_len=30, max_text_len=512):
        self.df = pd.read_csv(os.path.join(labeled_dir, f"{ticker}_{kline_type}_labeled.csv"), parse_dates=["Date"])
...
# 训练函数
def train_model(ticker, kline_type="weekly", seq_len=30, batch_size=16, num_epochs=10, lr=1e-4):
