...
for stock in stocks:
    ...
    df = yf.download(stock, start=start_date, end=end_date, interval="1wk")
    df = df[["Open", "High", "Low", "Close", "Volume"]]
    # "开" "高" "低" "收" "量"
    # 文件保存路径
    file_path = os.path.join(output_dir, f"{stock}_weekly.csv")
    
    # 保存数据
    df.to_csv(file_path)
    print(f"Saved {stock} data: {len(df)} weeks to {file_path}")